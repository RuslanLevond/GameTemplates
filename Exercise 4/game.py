...
def normalise_input(user_input):
    ...
    'help'
    """
    normali_text = remove_punct(user_input)
    normali_text = remove_spaces(normali_text)
    normali_text = normali_text.lower()
    return normali_text
...
def exit_leads_to(exits, direction):
    ...
    'Reception'
    """
    exit_leads = exits[direction]
    exit_leads = rooms[exit_leads]["name"]
    return exit_leads
...
def print_menu(exits):
    ...
    for key in exits:
       print_menu_line(key, str(exit_leads_to(exits, key)))
    # COMPLETE THIS PART:
...
def menu(exits):
    ...
    while True:
        print_menu(exits)
        users_input = input()
        users_input = normalise_input(users_input)
        if(is_valid_exit(exits, users_input) == False):
            print("\nThis path is unavailable, please enter one of the available paths!\n")
        else:
            return users_input
...
def move(exits, direction):
    ...
    False
    """
    coords = [exits][0][direction]
    new_room = rooms[coords]
    return new_room
